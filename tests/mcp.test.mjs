...
test("tools/list returns all seventeen calculators with input schemas and read-only annotations", async () => {
  const j = await (await mcp({ jsonrpc: "2.0", id: 2, method: "tools/list" })).json();
  assert.equal(j.result.tools.length, 17);
...
test("tools/call reaches the new calculators (fire-number)", async () => {
  const j = await (await mcp({ jsonrpc: "2.0", id: 7, method: "tools/call", params: { name: "fire-number", arguments: { annualSpend: 40000 } } })).json();
