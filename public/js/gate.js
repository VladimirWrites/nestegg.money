...
(()=>{const rU=()=>{scheduleSync();renderRetire();};const r=()=>retCfg(),cy=new Date().getFullYear();
  ...
  const pm=document.getElementById("rtPmode");if(pm)pm.onchange=e=>{r().pmode=e.target.value==="de"?"de":"amount";retSyncInputs();rU();};
  const pen=document.getElementById("rtPension");if(pen)pen.oninput=e=>{r().pension=parseFloat(e.target.value)||0;rU();};
