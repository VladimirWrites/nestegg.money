...
(()=>{const rU=()=>{scheduleSync();renderRetire();};const r=()=>retCfg();
  const on=document.getElementById("rtOn");if(on)on.onchange=e=>{r().on=e.target.checked;rU();};
  const yr=document.getElementById("rtYear");if(yr)yr.oninput=e=>{r().year=parseInt(e.target.value,10)||new Date().getFullYear();rU();};
  ...
  const wm=document.getElementById("rtWrMode");if(wm)wm.onchange=e=>{r().wrMode=e.target.value==="years"?"years":"rate";retSyncInputs();rU();};
  const wr=document.getElementById("rtWrRate");if(wr)wr.oninput=e=>{r().wrRate=Math.min(Math.max((parseFloat(e.target.value)||0)/100,0),0.2);rU();};
  const yrs=document.getElementById("rtYears");if(yrs)yrs.oninput=e=>{r().years=Math.min(Math.max(parseInt(e.target.value,10)||1,1),70);rU();};
})();
