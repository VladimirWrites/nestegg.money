...
(()=>{const fcU=()=>{scheduleSync();renderForecast();};
  ...
  const pyr=document.getElementById("fcPtsYr");if(pyr)pyr.oninput=e=>{pen().ptsPerYear=parseFloat(e.target.value)||0;fcU();};
  const pval=document.getElementById("fcPtVal");if(pval)pval.oninput=e=>{pen().ptValue=parseFloat(e.target.value)||0;fcU();};
  const pstt=document.getElementById("fcPtStart");if(pstt)pstt.oninput=e=>{pen().startYear=parseInt(e.target.value,10)||new Date().getFullYear();fcU();};
})();
